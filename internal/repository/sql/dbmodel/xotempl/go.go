...
func buildQueryType(ctx context.Context, query xo.Query) (Table, error) {
	...
	for _, z := range query.Fields {
		f, err := convertField(ctx, tf, z, query.Fields)
		if err != nil {
			return Table{}, err
		}
...
func convertTable(ctx context.Context, t xo.Table) (Table, error) {
	...
	for _, z := range t.Columns {
		f, err := convertField(ctx, camelExport, z, t.Columns)
		if err != nil {
			return Table{}, err
		}
...
func convertIndex(ctx context.Context, t Table, i xo.Index) (Index, error) {
	...
	for _, z := range i.Fields {
		f, err := convertField(ctx, camelExport, z, i.Fields)
		if err != nil {
			return Index{}, err
		}
		fields = append(fields, f)
...
func convertFKey(ctx context.Context, t Table, fk xo.ForeignKey) (ForeignKey, error) {
	...
	for _, f := range fk.Fields {
		field, err := convertField(ctx, camelExport, f, fk.Fields)
		if err != nil {
	...
	for _, f := range fk.RefFields {
		refField, err := convertField(ctx, camelExport, f, fk.RefFields)
		if err != nil {
			return ForeignKey{}, err
		}
...
func convertField(ctx context.Context, tf transformFunc, f xo.Field, fields []xo.Field) (Field, error) {
	...
	if f.IsPrimary {
		...
		for _, fld := range fields {
			if fld.IsPrimary && fld.Name != f.Name {
				isSingleKey = false
				break
			}
		}
...
func (f *Funcs) FuncMap() template.FuncMap {
	return template.FuncMap{
		...
		"func_context_slice":  f.func_context_slice,
		"func":                f.func_none,
...
func (f *Funcs) funcfn(name string, context, returnSlice bool, v interface{}) string {
	var p, r []string
	if context {
	...
	case Index:
		// params
		if x.IsSingleKey && returnSlice {
			p = append(p, f.params(x.Fields, false)+" []"+f.typefn(x.Fields[0].Type))
		} else {
			p = append(p, f.params(x.Fields, true))
		}
		// returns
		rt := "*" + x.Table.GoName
		if !x.IsUnique || returnSlice {
			rt = "[]" + rt
		}
		r = append(r, rt)
	default:
...
func (f *Funcs) func_context(v interface{}) string {
	return f.funcfn(f.func_name_context(v), f.contextfn(), false, v)
}

func (f *Funcs) func_context_slice(v interface{}) string {
	return f.funcfn(f.func_name_context(v)+"s", f.contextfn(), true, v)
}

// func_none genarates a func signature for v without context.
func (f *Funcs) func_none(v interface{}) string {
	return f.funcfn(f.func_name_none(v), false, false, v)
}
...
func (f *Funcs) db_prefix(name string, skip, byRef bool, vs ...interface{}) string {
	var prefix string
	...
	for i, v := range vs {
		...
		case Table:
			...
			p := f.names_ignore(prefix, v, byRef, ignore...)
...
func (f *Funcs) db_update(name string, v interface{}) string {
	...
	case Table:
		...
		p = append(p, f.names_ignore(prefix, x, true, ignore...), f.names(prefix, x.PrimaryKeys))
	default:
...
func (f *Funcs) logf(v interface{}, ignore ...interface{}) string {
	...
	switch x := v.(type) {
	case Table:
		p = append(p, f.names_ignore(f.short(x.GoName)+".", x, false, ignoreNames...))
	default:
...
func (f *Funcs) logf_update(v interface{}) string {
	...
	case Table:
		...
		p = append(p, f.names_ignore(prefix, x, false, ignore...), f.names(prefix, x.PrimaryKeys))
	default:
...
func (f *Funcs) namesfn(all bool, prefix string, byRef bool, z ...interface{}) string {
	var names []string
	for i, v := range z {
		switch x := v.(type) {
		case string:
		...
		case []Field:
			...
			for _, p := range x {
				fn := prefix + checkName(p.GoName)
				_, ignore := ignoreTypes[p.Type]
				if byRef && p.IsNullable && !p.IsArray && fn[0] != '&' && !ignore {
					fn = fmt.Sprintf("lo.Ternary(%s.Valid == false, nil, &%s)", fn, fn)
				}
...
func (f *Funcs) names_ignore(prefix string, v interface{}, byRef bool, ignore ...string) string {
	...
	return f.namesfn(true, prefix, byRef, vals)
}
...
func (f *Funcs) sqlstr_index_slice(v interface{}) []string {
	switch x := v.(type) {
	case Index:
		// build table fieldnames
		var fields []string
		...
		for i, z := range x.Fields {
			list = append(list, fmt.Sprintf("%s = ANY(%s)", f.colname(z), f.nth(i)))
			orderBy = append(orderBy, f.colname(z))
		}
		return []string{
			"SELECT ",
			strings.Join(fields, ", ") + " ",
...
func (f *Funcs) convertTypes(fkey ForeignKey) string {
	...
	for i := range fkey.Fields {
		...
		// извлечение значений из колонок, которые могут быть null
		switch typ {
		case "pgtype.Int8":
			expr = expr + ".Int64"
		case "pgtype.Int4":
...
type Index struct {
	...
	IsPrimary   bool
	Comment     string
	IsSingleKey bool // true if the field is a single primary key
}
