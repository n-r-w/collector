...
{{- else }}
	...
	for rows.Next() {
		...
		// scan
		if err := rows.Scan({{ names_ignore (print "&" (short $i.Table) ".") $i.Table true }}); err != nil {
			return nil, logerror(err)
		}
...
{{- if $i.IsSingleKey }}
// {{ func_name_context $i }}s retrieves a row from '{{ schema $i.Table.SQLName }}' as a [{{ $i.Table.GoName }}].
//
// Generated from index '{{ $i.SQLName }}'.
{{ func_context_slice $i }} {
	// query
	{{ sqlstr "index_slice" $i }}
	// run
	logf(sqlstr, {{ params $i.Fields false }})
...
{{ if (driver "postgres") -}}
	if err := {{ db_prefix "QueryRow" true true $t }}.Scan(&{{ short $t }}.{{ (index $t.PrimaryKeys 0).GoName }}); err != nil {
		return logerror(err)
	}	
...
{{- else -}}
	res, err := {{ db_prefix "Exec" true false $t }}
	if err != nil {
		return logerror(err)
	}
	// retrieve id
...
{{ recv_context $t "Upsert" }} {
	...
	// run
	{{ logf $t }}
	if _, err := {{ db_prefix "Exec" false true $t }}; err != nil {
		return logerror(err)
	}
	// set exists
	{{ short $t }}._exists = true
